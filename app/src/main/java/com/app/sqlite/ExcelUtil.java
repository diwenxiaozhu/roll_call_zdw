        ...
        if (objList != null && objList.size() > 0) {
            ...
            try {
                ...
                for (int j = 0; j < objList.size(); j++) {
                    Checkin checkin = (Checkin) objList.get(j);
                    List<String> list = new ArrayList<>();
                    list.add(checkin.getId() + "");
                    list.add(checkin.getStudentId());
                    list.add(checkin.getName());
                    list.add(checkin.getTime());
                    list.add(checkin.getType());
                ...
                writebook.write();
                workbook.close();
                Toast.makeText(c, "导出Excel成功,文件在根目录下，" + fileName, Toast.LENGTH_SHORT).show();
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                if (writebook != null) {
