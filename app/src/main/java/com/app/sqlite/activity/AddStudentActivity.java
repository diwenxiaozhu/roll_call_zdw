    ...
    private void initEvent() {
        ...
        btnSave.setOnClickListener(v -> {
            ...
            if (!TextUtils.isEmpty(imagePath)) {
                studentinfo = new Studentinfo(etId.getText().toString().trim(),
            ...
            boolean insert = studentinfoD.insert(studentinfo);
            if (insert) {
                SmartToast.show("添加成功");
            } else {
                SmartToast.show("添加失败，该学生已存在");
            }
        });

        imageView.setOnClickListener(v -> AndPermission.with(AddStudentActivity.this)
                .runtime()
